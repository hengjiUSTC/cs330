...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        for i, task in enumerate(task_batch):
            ...
            images_support = images_support.to(self.device) # (nus_classes * num_support, 1, 28, 28)
            labels_support = labels_support.to(self.device) # (nus_classes * num_support)
            images_query = images_query.to(self.device) # (nus_classes * num_query, 1, 28, 28)
            labels_query = labels_query.to(self.device) # (nus_classes * num_query)
            ...
            # accuracy_query_batch.
            support_features = self._network(images_support) # (nus_classes * num_support, 64)
            query_features = self._network(images_query) # (nus_classes * num_query, 64)
            
            # features_unique_labels = torch.unique(labels_support)

            # prototypes = []
            # for label in features_unique_labels:
            #     prototype_support = torch.mean(support_features[labels_support == label], dim=0) # (1, 64)
            #     prototypes.append(prototype_support)
            # prototypes_support = torch.stack(prototypes, dim=0) # (nus_classes, 64)

            features_unique_labels, inverse_indices = torch.unique(labels_support, return_inverse=True)
            prototypes_support = torch.zeros(len(features_unique_labels), support_features.size(1))

            prototypes_support.index_add_(0, inverse_indices, support_features)
            counts = torch.bincount(inverse_indices, minlength=len(features_unique_labels)).type_as(prototypes_support).view(-1, 1)
            prototypes_support = prototypes_support / counts


            query_prototype_distances = torch.cdist(query_features, prototypes_support)
            support_prototype_distances = torch.cdist(support_features, prototypes_support)
