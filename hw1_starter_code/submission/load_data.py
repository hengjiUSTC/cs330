...
class DataGenerator(IterableDataset):
    ...
    def _sample(self):
        ...
        class_folders = random.sample(self.folders, self.num_classes)
        labels_and_images = get_images(class_folders, range(self.num_classes), num_samples=self.num_samples_per_class)
        result_images = []
        result_labels = []
        for i in range(self.num_samples_per_class):
            ...
            for j in range(self.num_classes):
                label, image = labels_and_images[j*self.num_samples_per_class+i]
                images.append(self.image_file_to_array(image, self.dim_input))
                labels.append(np.eye(self.num_classes)[label])
        ...
        # Shuffle only the last row (k-1) of both images and labels
        indices = np.arange(self.num_classes)
        np.random.shuffle(indices)
        result_images[-1] = result_images[-1, indices, :]
        result_labels[-1] = result_labels[-1, indices, :]
        
        # print(result_images.shape, result_labels.shape)
        # print(result_labels)
        # pdb.set_trace()
